...
size_t findOfsOtherEnd (int fileNo, size_t offset, char *readName)
{
	...
	size_t delta = fileSize[fileNo]*10/1000;
...
//				printf ("Ofs %lu, interleaved %d\n", fI.offset+j, interleaved);
				if (interleaved)
				{	
					char rName[200], rName2[200];
					for (k=j; ((readChunk[k]!='/') && (readChunk[k]!=' ')); k++)
					{
						rName[k-j] = readChunk[k];
					}
					rName[k-j] = '\0';
					while (readChunk[k]!='\n') k++; k++;
					while (readChunk[k]!='\n') k++; k++;
					...
					while (readChunk[k]!='\n') k++; k++;
					int j2 =k;
					while ((readChunk[k]!=' ') && (readChunk[k]!='/')) 
					{
						rName2[k-j2]=readChunk[k];
						k++;
					}
					rName2[k-j2] = '\0';
					printf ("rName1 %s; rname2 %s\n", rName, rName2);
					if (strcmp (rName, rName2))
					{
						j=j2;
					}
					break;
